...
def shopping_list():
    ...
    with db_connection() as db:
        ...
        favorites = db.execute("""
            SELECT id, name, quantity, category, barcode
            FROM favorites
            WHERE list_id = %s
            ORDER BY LOWER(name)
            LIMIT 20
        ...
        history = db.execute("""
            SELECT id, name, quantity, category, barcode, purchased_at
...
def toggle_favorite(item_id):
    ...
    with db_connection() as db:
        item = db.execute("""
            SELECT name, quantity, category, barcode
            FROM items
            WHERE id = %s AND list_id = %s
    ...
    with db_connection() as db:
        ...
        if favorite:
            ...
            """, (
                ...
                favorite["category"], favorite["barcode"], user["id"]
            ))
        ...
        for item in completed_items:
            db.execute("""
                INSERT INTO purchase_history (
                    list_id, name, quantity, category, barcode, purchased_by
                )
                VALUES (%s, %s, %s, %s, %s, %s)
            """, (
                user["list_id"], item["name"], item["quantity"],
                item["category"], item["barcode"], user["id"]
            ))
    ...
    with db_connection() as db:
        ...
        if entry:
            ...
            """, (
                ...
                entry["category"], entry["barcode"], user["id"]
            ))
