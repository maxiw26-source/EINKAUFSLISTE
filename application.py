...
@app.route("/kassenbon", methods=["GET", "POST"])
@login_required
def receipt_upload():
    user = current_user()
    ...
    if request.method == "POST":
        ...
        if not receipt_file or not receipt_file.filename:
            flash("Bitte ein Bild des Kassenbons auswählen.", "error")
            return redirect(url_for("receipt_upload"))

        content_type = (receipt_file.mimetype or "").lower()

        if content_type not in ALLOWED_RECEIPT_TYPES:
            flash(
                "Unterstützt werden JPG, PNG, WEBP und HEIC/HEIF.",
                "error"
            )
            return redirect(url_for("receipt_upload"))

        receipt_file.stream.seek(0, 2)
        file_size = receipt_file.stream.tell()
        receipt_file.stream.seek(0)

        if file_size <= 0 or file_size > MAX_RECEIPT_FILE_SIZE:
            flash("Das Bild darf höchstens 8 MB groß sein.", "error")
            return redirect(url_for("receipt_upload"))

        # V12.0 speichert das Bild bewusst noch nicht dauerhaft.
        # OCR und sichere temporäre Speicherung folgen in V12.1.
        flash(
            "Bild wurde geprüft. Die OCR-Auswertung folgt im nächsten Update.",
            "success"
        )
        return redirect(url_for("receipt_upload"))
    ...
    <div class="navigation">
        ...
        <div class="top-actions">
            <a class="button blue" href="{{ url_for('dashboard') }}">
                📊 Dashboard
            </a>
            <a class="button gray" href="{{ url_for('shopping_list') }}">
                🛒 Liste
            </a>
        </div>
    </div>
    ...
    <div class="card">
        ...
        <p class="small">
            Fotografiere den Bon oder wähle ein Bild aus deiner Galerie.
        </p>

        <div class="scan-help">
            <div><span>📄</span><span>Bon vollständig ins Bild legen.</span></div>
            <div><span>💡</span><span>Gute Beleuchtung ohne Spiegelung.</span></div>
            <div><span>📐</span><span>Möglichst gerade von oben fotografieren.</span></div>
        </div>
    </div>

    <div class="card">
        <form class="receipt-upload"
              method="POST"
              enctype="multipart/form-data"
              onsubmit="return validateReceiptUpload();">

            <label class="receipt-dropzone" for="receipt-file">
                <div>
                    <strong>📷 Kamera oder Galerie öffnen</strong>
                    <span class="small">
                        JPG, PNG, WEBP oder HEIC · maximal 8 MB
                    </span>
                </div>
            ...
            <div id="receipt-preview" class="receipt-preview">
                <img id="receipt-preview-image"
                     alt="Vorschau des Kassenbons">

                <div id="receipt-file-info" class="small"></div>
    ...
    <div class="card">
        ...
        <p>
            In V12.1 liest die App den Bon automatisch aus und erkennt
            Artikel, Preise, Geschäft, Datum und Gesamtsumme.
        </p>
    </div>
    ...
    <script>
        ...
        function previewReceipt(event) {
            ...
            const info = document.getElementById("receipt-file-info");
            ...
            if (file.size > MAX_RECEIPT_SIZE) {
                alert("Das Bild ist größer als 8 MB.");
                clearReceiptPreview();
                return;
            }
