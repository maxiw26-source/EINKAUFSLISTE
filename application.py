...
def receipt_upload():
    ...
    <div class="card"><div class="receipt-upload">
      <div class="receipt-actions">
        <label class="button primary full" for="receipt-camera">📷 Foto aufnehmen</label>
        <label class="button blue full" for="receipt-gallery">🖼️ Aus Galerie wählen</label>
      </div>
      <input id="receipt-camera" type="file" accept="image/jpeg,image/png,image/webp" capture="environment" onchange="previewReceipt(event)" hidden>
      <input id="receipt-gallery" type="file" accept="image/jpeg,image/png,image/webp" onchange="previewReceipt(event)" hidden>
      <div class="receipt-dropzone"><div><strong>Bonfoto auswählen</strong><span class="small">JPG, PNG oder WEBP · maximal 8 MB</span></div></div>
      ...
      <div id="ocr-progress" class="ocr-progress">
        <span class="small">
          Der erste Start kann auf dem iPhone 20–60 Sekunden dauern.
          Die App währenddessen geöffnet lassen.
        </span><strong id="ocr-status">OCR wird vorbereitet …</strong><div class="ocr-progress-track"><div id="ocr-progress-fill" class="ocr-progress-fill"></div></div><span id="ocr-progress-text" class="small">0 %</span></div>
    </div></div>
    ...
    <script>
      ...
      function clearReceiptPreview(){selectedReceiptFile=null; document.getElementById("receipt-camera").value="";document.getElementById("receipt-gallery").value=""; document.getElementById("receipt-preview").classList.remove("visible"); document.getElementById("receipt-review").classList.remove("visible"); if(receiptObjectUrl){URL.revokeObjectURL(receiptObjectUrl);receiptObjectUrl=null;}}
      ...
      async function runReceiptOcr(){
        if(!selectedReceiptFile){
          alert("Bitte zuerst ein Bonfoto auswählen.");
          return;
        }
        ...
        updateOcrProgress("OCR-Bibliothek wird geladen …",0.02);

        try{
          if(!window.Tesseract || typeof Tesseract.recognize!=="function"){
            throw new Error(
              "Die OCR-Bibliothek konnte nicht geladen werden. " +
              "Bitte Internetverbindung prüfen und die App vollständig schließen und neu öffnen."
            );
          }

          const timeoutPromise=new Promise((_,reject)=>{
            window.setTimeout(
              ()=>reject(new Error("OCR-Zeitüberschreitung. Bitte ein kleineres oder schärferes Bild versuchen.")),
              120000
            );
          });
            ...
            "deu",
            {
              logger:function(message){
                const labels={
                  ...
                  "recognizing text":"Bontext wird erkannt …"
                };

                updateOcrProgress(
                  labels[message.status]||message.status||"OCR läuft …",
                  message.progress||0
                );
              }
            }
          ...
          if(!text.trim()){
            throw new Error(
              "Es wurde kein Text erkannt. Bitte den Bon näher, gerade und gut beleuchtet fotografieren."
            );
          }

          document.getElementById("raw-ocr-text").value=text;
          renderReceiptReview(parseReceiptText(text));
          updateOcrProgress("OCR abgeschlossen.",1);
          ...
          const message=
            error && error.message
              ? error.message
              : "Der Bon konnte nicht erkannt werden.";

          alert(message);
          updateOcrProgress("OCR fehlgeschlagen: "+message,0);
